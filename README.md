...
Embed mode: grape jelly on a lavender sweep in front of the extruded Smashbar
wordmark; no furniture, no portal, no other worlds; the mute button, the
...
- `src/graphics/scene/sweep.ts`, `wordmark.ts`, `src/assets/smashbar-wordmark.svg` — the sweep ground and the extruded wordmark.
